...
def load_data():
  #create dataset
    train_ds = MidiDataset('./data', 'toy_data', MAX_CONTEXT, shuffle=False)
    valid_ds = MidiDataset('./data', 'toy_data', MAX_CONTEXT, shuffle=False)
    test_ds = MidiDataset('./data', 'toy_data', MAX_CONTEXT, shuffle=False)
...
def validate(model, val_loader):
  ...
  with torch.no_grad():
    for i, batch in enumerate(val_loader):
      ...
      logits = model.forward(self, x, z=z, labels=labels, 
                          position_ids=position_ids, bar_ids=bar_ids, 
                          description_bar_ids=description_bar_ids, 
                          return_hidden=False)
      pred = logits.view(-1, logits.shape[-1])
...
def train_val_loop(model):
  ...
  for epoch in range(1, EPOCHS+1):
    ...
    val_loss, val_ppl = validate(model, val_loader)
    print(f'train loss: {train_loss:.3f}, val loss: {val_loss:.3f}, val ppl: {val_ppl:.3f}')
    if val_loss < best_val_loss:
      best_val_loss = val_loss
      best_val_ppl = val_ppl
      best_epoch = epoch
      model.save_checkpoint(f'./checkpoints/{MODEL}.ckpt')
    scheduler.step()
